...
boardElement.addEventListener("pointercancel", resetPointer);
...
function applyDirectionVector(direction) {
    state = setDirection(state, direction);
...
function handlePointerUp(event) {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
        applyDirectionVector(deltaX > 0 ? { x: 1, y: 0 } : { x: -1, y: 0 });
        return;
    }

    applyDirectionVector(deltaY > 0 ? { x: 0, y: 1 } : { x: 0, y: -1 });
}
...
function registerServiceWorker() {
    ...
    window.addEventListener("load", () => {
        navigator.serviceWorker.register("./snake-sw.js").catch(() => {
            // Ignore registration failures; the game still works without offline mode.
        });
    });
}
